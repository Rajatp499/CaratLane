 ...
 const [value, setValue] = useState('');
  ...
  return (
    <>
      ...
      value={value}
      containerStyle={{padding:'0px', display:'block', width:'fit-content',borderRadius: '0.375rem 0 0 0.375rem',}}
      inputStyle = {{padding: '0.5rem',height: '3rem',fontSize:'1rem', width:'100%',color: 'rgb(148,163,184)'}}
      onChange={(value) => {
        setValue(value)
      }}
    />
            ...
            {menu[hovered].shopbymetalandstone.map((items) => {
              return (
                <div
                  ...
                  key={items.id}
                >
                  <items.image className={`m-3 text-${items.color}-600 `} />
                  {items.name}
                </div>
              );
            })}
